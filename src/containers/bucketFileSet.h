...
/* The BucketFileSet stores a (potentially huge) set of key-value pairs on disk.
   The keys have to be of type uint64_t, while the value can be of arbitrary type.
...
public:
    ...
    void clearBucket(uint64_t bucketId) {
        ...
        ftruncate( fileno(bucketFiles[bucketId]), 0);
    }
    ...
    uint64_t getNumBuckets() const { return bucketFiles.size();}
